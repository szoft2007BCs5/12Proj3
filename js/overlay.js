...
export function enableTemplate(selected){ //kiválasztott oldal betöltése
    ...
    if(selected == "settings"){ //settings oldal eseménykezelőinek hozzáadása
        document.getElementsByClassName("bt-settings")[0] //vissza a main oldalra
...
export function changeLanguage(selected = localStorage.getItem("currentLang")) {
    for (const e in window.lang) { //minden elemre a lang fájlban
        if (document.getElementById(e))
            document.getElementById(e).innerHTML = window.lang[e][selected.toLowerCase()];
    }
